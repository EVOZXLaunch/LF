...
// buy/sell tax itself is disabled. Getting this wrong is a
// guaranteed revert, so it is computed here in one place
// instead of trusting raw form input.
// =====================================================

function buildTaxShares(form) {

    const marketingWallet =
        validWallet(form.marketingWallet);

    const developmentWallet =
        validWallet(form.developmentWallet);

    let burnShare =
        Math.min(
            100,
            Math.max(0, normalizeNumber(form.burnTaxShare))
        );

    let marketingShare = 0;

    let developmentShare = 0;

    if (!marketingWallet && !developmentWallet) {

        // No receiver wallets configured — the only safe
        // configuration that always sums to 100 is sending
        // everything to burn.
        burnShare = 100;
        ...
        const remaining = 100 - burnShare;

        if (marketingWallet && developmentWallet) {

            marketingShare = Math.floor(remaining / 2);

            developmentShare = remaining - marketingShare;

        }

        else if (marketingWallet) {

            marketingShare = remaining;

        }

        else {

            developmentShare = remaining;
    ...
    return {
        ...
        marketingWallet: marketingWallet || ZeroAddress,

        developmentWallet: developmentWallet || ZeroAddress,

        treasuryWallet: ZeroAddress,

        liquidityWallet: ZeroAddress,

        buybackWallet: ZeroAddress,

        charityWallet: ZeroAddress
...
export async function buildDeployment(form) {
    ...
    // No explicit "max supply" field in the current UI —
    // default max supply to the initial supply unless
    // minting is enabled, in which case allow generous room
    // to grow without hitting MAX_SUPPLY unexpectedly.
    const maxSupply =
        form.mintable
            ? initialSupply * 10n
            : initialSupply;
